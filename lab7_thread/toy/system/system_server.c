...
int system_server()
{
    ...
    retcode = pthread_create(&watchdog_thread_tid, NULL, watchdog_thread,
                             "Watchdog Thread Created!\n");
    assert(retcode == 0);
    pthread_detach(watchdog_thread_tid);
    retcode = pthread_create(&monitor_thread_tid, NULL, monitor_thread,
                             "Monitor Thread Created!\n");
    assert(retcode == 0);
    pthread_detach(monitor_thread_tid);
    retcode = pthread_create(&disk_service_thread_tid, NULL, disk_service_thread,
                             "Disk Service Thread Created!\n");
    assert(retcode == 0);
    pthread_detach(disk_service_thread_tid);
    retcode = pthread_create(&camera_service_thread_tid, NULL, camera_service_thread,
                             "Camera Service Thread Created!\n");
    assert(retcode == 0);
